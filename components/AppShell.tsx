    ...
    <div className="flex h-screen">
      ...
      <aside className="sidebar-bg flex h-full w-56 shrink-0 flex-col overflow-y-auto">
        ...
        <div className="flex items-center gap-2.5 px-5 py-5">
          <span className="flex items-center rounded-lg bg-white px-2 py-1.5 shadow-sm">
            <Image src="/logo.png" alt="R&R" width={80} height={47} className="h-5 w-auto" priority />
          </span>
          <span className="text-sm font-bold text-white/90">AI Visibility</span>
        </div>
        ...
        <div className="border-t border-white/10 px-3 py-4">
          <button
            ...
            className="flex w-full items-center gap-3 rounded-lg px-3 py-2.5 text-sm text-white/60 transition hover:bg-white/8 hover:text-white/90"
          >
            <LogoutIcon />
            Sign out
          </button>
...
function ProjectLinks({ pathname }: { pathname: string }) {
  // Extract project ID from path like /projects/[id] or /projects/[id]/dashboard
  ...
  return (
    <div className="mt-4 border-t border-white/10 pt-4">
      <p className="mb-2 px-3 text-[10px] font-semibold tracking-[0.12em] text-white/40 uppercase">
        Active Project
      </p>
      {links.map((item) => {
...
function DashboardIcon({ active }: { active?: boolean }) {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className={active ? "text-white" : "text-white/50"}>
...
function LogoutIcon() {
  return (
    <svg width="18" height="18" viewBox="0 0 18 18" fill="none" className="text-white/50">
